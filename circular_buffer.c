...
void StaticQueue_init(StaticQueue* queue,uint8_t size)
{
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
	{
		queue = malloc(sizeof(byte_t) * size);
		queue->current_push_pos_ = 0;
		queue->current_pop_pos_  = 0;
		queue->size_ = size;
		queue->overdistance_ = 0;
	}
}

//pushing is always possible 
void StaticQueue_pushByte(StaticQueue* queue,byte_t val)
{
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
	{
		queue->body_[queue->current_push_pos_] = val;
		if( queue->current_push_pos_ ==  ( queue->size_ - 1 ) )
		{
			++queue->overdistance_;
			queue->current_push_pos_ = 0;
		}
		else ++queue->current_push_pos_;
	}
}

//pop if queue is not empty
byte_t StaticQueue_popByte(StaticQueue* queue)
{
	byte_t val = 0;
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
	{
		if(!StaticQueue_isEmpty(queue))
		{
			val = queue->body_[queue->current_pop_pos_];
			if( queue->current_pop_pos_ ==  ( queue->size_ - 1 ) )
			{
				--queue->overdistance_;
				queue->current_pop_pos_ = 0;
			}
			else ++queue->current_pop_pos_;
		}
	}
	return val;
}
